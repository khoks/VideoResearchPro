...
**Status.** 🟡 in-progress as of 2026-05-04. **E-5.1 (tenancy retrofit) fully closed** across PR #149-#152 + the phase 2c runbook: phase 0 audit doc, phase 1 additive nullable `tenant_id` columns + indexes on `jobs` / `qa_exchanges` / `library_qa_exchanges` / `qa_history_exchanges`, phase 2a backfill migration + write-side stamping, phase 2b read-side filtering with cross-tenant 404 isolation, phase 2c operator-coordinated NOT NULL runbook ([`docs/migration-tenant-id-not-null.md`](migration-tenant-id-not-null.md) + Alembic migration shipped, operator-applied per the D-032 precedent). **E-5.2 through E-5.9 still ⚪** (subscription tiers, billing / Stripe, abuse prevention, background-job isolation, data residency, hosting, hosted UX, auth hardening — OAuth / MFA / password reset / audit log). Detailed in [saas-roadmap.md](saas-roadmap.md).
