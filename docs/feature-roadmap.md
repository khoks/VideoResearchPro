...
- 🟢 **Backend reference enrichment — per-document layer shipped 2026-05-03** ([PR #131](https://github.com/khoks/VideoResearchPro/pull/131)). Threaded `source_type` / `source_id` / `source_url` / `permalink` / `author` / `subreddit` / `instance` from `Document.source_metadata_json` through `_build_video_metadata()` → `chunk_transcript()` → Chroma upserts. **Production citations across all 5 source types now render polymorphically**, with OP-level permalinks. Per [D-030](decisions.md#d-030--backend-reference-enrichment-ships-per-document-polymorphic-chroma-metadata-first-per-segment-comment_idcomment_url-deferred-2026-05-03), per-segment fields (`comment_id` / `comment_url` for reply-anchor deep-links) are deferred — they need a structural change to `chunk_transcript()` to preserve per-segment `extra` through sentence-expansion + greedy-pack + dominant-reply collapse. The connector flatten layer already emits them; the chunker just strips them today.
- 🔵 **Next milestones**: M-1.7 (podcast end-to-end + Whisper-as-service decision per [OQ-4](initiatives.md#oq-4)), M-2.5 (marketing landing page deployed). M-1.5/M-1.6 polish backlog top items: **per-segment reply-anchor citations** (chunker rework noted above), `extract_references` LLM-prompt update to teach it the polymorphic shape (empirical reference quality is fine on non-video chunks today because `title` is always populated, but a prompt tweak would sharpen it), HTTP-level integration tests (current e2e tests drive Celery `.run()` directly).
- 🔵 **Then: remaining E-1.5 surfaces.** Mode B paste mode covering FB / IG / LI / X-without-paid-API, then BYOK Twitter API ([D-009](decisions.md#d-009--twitter--x-is-byok--opt-in-2026-04-25)). Discord and TikTok deferred ([D-010](decisions.md#d-010--defer-tiktok-and-per-server-discord-bot-indefinitely-2026-04-25)). All slot into the same polymorphic plumbing M-1.5 / M-1.6 validated.
