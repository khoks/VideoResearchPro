...
- 🟢 **Article-extraction primitives shipped 2026-05-03** ([PR #135](https://github.com/khoks/VideoResearchPro/pull/135), T-1.6.1). `app/services/article_extraction/` — `ExtractionResult` + `extract_text(url)` hybrid orchestrator (httpx → trafilatura → Playwright stub fallback). 20 unit tests; backend suite 593 passing. **Unblocks S-1.5.8 Mode B paste-mode** immediately; full article-connector UX (T-1.6.2 → T-1.6.5) deferred per [D-024](decisions.md#d-024--flip-e-16-to--with-primitives-only-scope-split-2026-04-28) until after M-1.7.
...
- 🔵 **Next milestones**: M-1.7 (podcast end-to-end + Whisper-as-service decision per [OQ-4](initiatives.md#oq-4)), then E-1.6 full article UX, then E-1.8 (PDF / e-book), then E-1.9 (channels → creators rename migration).
