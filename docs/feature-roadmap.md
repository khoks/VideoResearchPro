...
- 🟢 **Reddit + HN candidate connectors shipped standalone (S-1.5.1 PR #70, S-1.5.2 PR #73).** Both emit `Candidate` objects with namespaced `source_id` (`reddit:abc`, `hn:42M`); pseudo-timestamp synthesis at 3 wps codified in [D-013](decisions.md#d-013--pseudo-timestamp-synthesis-at-3-wps-as-a-shared-cross-source-constant-2026-04-26).
...
- 🟡 **I-5 SaaS readiness — E-5.1 substantially closed 2026-05-04.** Four PRs ([#149](https://github.com/khoks/VideoResearchPro/pull/149) audit / [#150](https://github.com/khoks/VideoResearchPro/pull/150) phase 1 / [#151](https://github.com/khoks/VideoResearchPro/pull/151) phase 2a / [#152](https://github.com/khoks/VideoResearchPro/pull/152) phase 2b) take the codebase from **structurally single-tenant** (zero `tenant_id` on user-scoped tables) to **fully filtered reads + writes**. Cross-tenant isolation enforced via 404-not-403 on the four user-scoped tables. Phase 2c (NOT NULL constraint) deferred to operator runbook per [D-032](decisions.md#d-032--operator-coordinated-runbook-vs-automatic-startup-migration-for-data-bearing-identifier-renames-2026-05-03) precedent. **E-5.2 through E-5.9 still ⚪** — subscription tiers, Stripe billing, abuse prevention, background-job isolation, data residency, hosting/infra, hosted UX, auth hardening (OAuth/MFA/audit log).
...
**Status.** 🟡 in-progress as of 2026-05-04. **E-5.1 (tenancy retrofit) substantially closed** through PR #149-#152: phase 0 audit doc, phase 1 additive nullable `tenant_id` columns + indexes on `jobs` / `qa_exchanges` / `library_qa_exchanges` / `qa_history_exchanges`, phase 2a backfill migration + write-side stamping, phase 2b read-side filtering with cross-tenant 404 isolation. Phase 2c (NOT NULL constraint) is deferred to an operator runbook — only the operator can prove zero-NULL after their backfill runs. **E-5.2 through E-5.9 still ⚪** (subscription tiers, billing / Stripe, abuse prevention, background-job isolation, data residency, hosting, hosted UX, auth hardening — OAuth / MFA / password reset / audit log). Detailed in [saas-roadmap.md](saas-roadmap.md).
