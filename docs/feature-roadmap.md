...
- 🔵 **Next milestones**: M-1.7 (podcast end-to-end + Whisper-as-service decision per [OQ-4](initiatives.md#oq-4)), M-2.5 (marketing landing page deployed). M-1.6 closure also unblocks the M-1.5 polish backlog — the highest-value remaining item is **backend reference enrichment**: teach the `extract_references` LLM use case + the chunking pipeline to write `source_type` / `permalink` / `author` / `comment_url` per chunk so Mastodon / Bluesky / Reddit / HN citations actually flow with their proper labels in production. Frontend rendering contract ([PR #117](https://github.com/khoks/VideoResearchPro/pull/117) + extensions in #128 / #129) is in place; the backend producer is the missing half.
