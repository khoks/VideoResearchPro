...
- **S-1.14.13 🟢 `search_rank_and_curate` quotas — shipped 2026-07-30.** Linked decisions: [D-059](decisions.md#d-059--selection-quotas-preferred-channel-guarantee--concentration-cap-2026-07-30), [D-060](decisions.md#d-060--extraction-ratio-raised-020--040-2026-07-30). Validated on the real 651-candidate pool: preferred coverage **12/29 → 29/29**, largest channel share **10.5% → 8.0%**, distinct channels **86 → 103**.
  - [x] T-1.14.13.1 Preferred-channel guarantee (deterministic; the prompt already asked and did not comply). Draws from the full pool so early-round eliminations are recovered
  - [x] T-1.14.13.2 Per-channel cap `max(3, 8% of target)`, relaxing only when too few channels exist to fill the target
  - [x] T-1.14.13.3 A/B'd against the shipped 200 on the captured pool — the measurement S-1.14.6 bought
  - [x] T-1.14.13.4 Sub-120s clips pruned when the user set no floor, **preferred channels exempt** (filtering them cost 6 of 29 their only candidates). ~~non-English title filter~~ — **rejected**: contradicts the multilingual design (see D-059)
  - [x] T-1.14.13.5 `_MAP_EXTRACTION_RATIO` 0.20 → 0.40 (D-060): 0.20 capped extraction at 288K vs the control's 609K, so our own budget was the ceiling once Luna landed
