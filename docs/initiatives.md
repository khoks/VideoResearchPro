...
- **S-1.14.14 🔵 Close the residual report gap — all three defects are prompt/retry, not model tier.**
  - [ ] T-1.14.14.1 `COMPOSE_SECTION_PROMPT`: require the source name in the VISIBLE citation text (`Title — Channel · mm:ss`), not just in the href. 75% of production citations render as a bare `( 12:34 )`; scored 6 vs 9 on attribution despite all 1,489 anchors carrying correct URLs
  - [ ] T-1.14.14.2 `COMPOSE_SECTION_PROMPT`: organise Key Facts by THEME and Analysis by named debate, instead of the current per-channel digest — the single largest score gap (insight 5 vs 9)
  - [ ] T-1.14.14.3 Retry transient 5xx in `compose_report` with backoff; a `529 Overloaded` cost an entire report section
  - [ ] T-1.14.14.4 Feed real statistics into the executive-summary prompt — a judge caught production mis-stating corpus size and vintage
  - [ ] T-1.14.14.5 Re-judge after the above, and only then decide whether `report_compose` warrants a model-tier change
