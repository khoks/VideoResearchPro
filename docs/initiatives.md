...
- **OQ-9.** Master branch-protection policy vs autonomous-merge sessions. During the 2026-04-26 session, `required_approving_review_count: 1` was active on `master`, blocking autonomous squash-merges; PRs [#85](https://github.com/khoks/VideoResearchPro/pull/85) / [#86](https://github.com/khoks/VideoResearchPro/pull/86) (and presumably future persistence PRs) require `gh pr merge --admin` to bypass. The user's [feedback memory `feedback_pr_workflow.md`](https://github.com/khoks/VideoResearchPro) explicitly directs autonomous merge without review pings. Three resolutions:
  - **(a)** Drop `required_approving_review_count` entirely — simplest; loses the audit-trail breadcrumb that someone (admin) explicitly bypassed review.
  - **(b)** Keep the rule, accept `--admin` overrides as the operational pattern. Preserves the audit trail (every bypass is recorded as an admin override on the PR); still autonomous from the user's perspective.
  - **(c)** Configure a branch-protection bypass list (or CODEOWNERS rule) that includes the merging account, so review-required is enforced for *humans* but not for the autonomous skill account.
  - Recommendation: **(b)** — cheapest, preserves the audit trail, and matches the user's "don't ask me" directive while still leaving a paper trail. To be confirmed next session.
