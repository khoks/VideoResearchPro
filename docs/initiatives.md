...
- **OQ-7.** E-1.10 migration cadence — **hard cutover** (drop legacy `video_id` PK + add `(source_type, source_id)` unique constraint in one Alembic migration) vs. **additive-then-cutover** (ship `document_id` UUID + `source_id text` alongside the legacy `video_id` for one release with dual-write / dual-read, then drop `video_id` in a follow-up PR). Hard cutover is one PR but irreversible the moment any reader misses the rename. Additive-then-cutover costs an extra PR but buys reversibility against missed call sites — meaningful given the project ships SQLite (no `pg_dump`-and-restore parachute) and the 14 importers across `youtube_service`, `chroma_service`, agents, routers, and exports. Recommendation pending: additive-then-cutover. Decide before T-1.10.1 lands. (Tied to [E-1.10](#e-110-🔵-promote-video_id-pk-to-uuid-document_id), [D-015](decisions.md#d-015--promote-e-110-uuid-pk-ahead-of-reddit--hn-orchestrator-wiring-2026-04-26))
