...
- **S-1.14.10 🔵 Re-run the E-1.14 blind harness after the funnel fix** to separate residual *model* delta from the (now removed) plumbing delta. The D-055 report verdict was measured against the starved pipeline; re-judging is what tells us whether a tier upgrade is still warranted for `report_compose`.
