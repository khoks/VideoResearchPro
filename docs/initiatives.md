...
- **OQ-8.** Polymorphic `<ApprovalCard>` TypeScript shape — four interrelated sub-questions tied to T-1.5.4.1. Sketch in [source-types.md § Polymorphic ApprovalCard TypeScript shape](source-types.md#polymorphic-approvalcard-typescript-shape-proposed-2026-04-26). To be answered before T-1.5.4.1 (build polymorphic primitive) starts.
  - **(a) Where does `SourceMetadata` live?** Build step (Pydantic → `json-schema-to-typescript` → frontend `.d.ts`) vs hand-rolled-and-synced. Build step = one-time setup, drift structurally impossible. Hand-rolled = zero-config, PR review is the only enforcement. Recommendation: **build step** — drift across a 19-use-case + N-source-type schema becomes painful fast.
  - **(b) Field references.** Document-level fields like `title` / `published_at` / `source_url` aren't in `source_metadata` — they're on the row. Card needs both. Options: flat `keyof (Document & T)` with a `View<T> = Document & T` alias (ergonomic; chip says `field: 'title'`), or namespaced `'document.title' | 'metadata.subreddit'` (explicit; verbose). Recommendation: **flat with `View<T>` alias**.
  - **(c) Formatters.** Per-chip `format` callback vs typed registry (e.g. `durationSeconds → "12:34"`, `epochMs → "3h ago"`, `signedNumber → "+42"`) vs hybrid. Recommendation: **hybrid** — registry for the ~6 common cases, callback override for one-offs.
  - **(d) Filter chips.** Share `MetaChip` with a `filterable: 'eq' | 'gte' | 'lt' | 'contains'` discriminator (one chip = both display + filter, declared once) vs separate `FilterChip` type (cleaner types, duplicated registration). Recommendation: **shared**.
  - (Tied to [S-1.5.4](#s-154-🔵-single-polymorphic-approvalcard-per-d-016), [D-016](decisions.md#d-016--single-polymorphic-approval-card-driven-by-source_metadata-2026-04-26))
