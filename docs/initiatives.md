...
- **S-1.14.8 🔵 Fix the report information funnel — highest-value follow-up, model-independent.** The shipped report cites 2 channels / 4 videos out of 200 while claiming 137. Caps starve the pipeline: map `max_tokens=3000` against ~116K-token batches (registry still sizes this call at `p95_input=32,000` — 3.6× stale), reduce `max_tokens=6000` applied every pairwise round, so ≤0.43% of source tokens reach compose. A control run found **zero duplicates** in the map output, so any item-count reduction in reduce is pure loss. `compose` resolves to claude-sonnet-5 (1M window) yet is bound by a self-imposed 120K `_QUALITY_BATCH_CAP`.
  - [ ] T-1.14.8.1 Rescale `report_map_chunks` `max_tokens` to the real batch size (3,000 → ~20,000) and correct the registry's stale `p95_input` / `typical_output_tokens`
  - [ ] T-1.14.8.2 Scale reduce's per-round budget instead of a flat 6,000; compress per-video evenly so no video is dropped wholesale
  - [ ] T-1.14.8.3 Raise `_QUALITY_BATCH_CAP` for `report_compose` (sonnet-5's window makes 120K arbitrary), or stage compose over sections
  - [ ] T-1.14.8.4 Account for and surface reduce/compose drops the way S-1.12.4 did for map failures — silent narrowing must become visible
  - [ ] T-1.14.8.5 Re-run the E-1.14 harness after the fix to isolate residual *model* delta from the funnel delta
- **S-1.14.9 🔵 Two corpus traps found during replay.** (a) All 5,861 map items carry an empty `video_url` — chunk headers omit URLs, so timestamp citations can't link without a join back to `documents`. (b) The corpus holds two pairs of distinct videos whose titles differ only in letter case; any step keying identity on `video_title` case-insensitively undercounts by 2.
