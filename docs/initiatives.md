...
- **S-1.14.10 🟡 Re-run the E-1.14 harness after the funnel fix.** Report regenerated on the reference corpus 2026-07-30 — see [D-058](decisions.md#d-058--report-pipeline-fix-validated-on-the-reference-job-2026-07-30): body words 5,610 → 25,692, videos linked 0 → 125, channels 67 → 92/92, dead links 143 → 0, quantitative claims 0 → 130. **Blind re-judging still outstanding** — the D-055 verdict was measured against the starved pipeline, so a fresh A/B is what tells us whether `report_compose` still warrants a tier change.
- **S-1.14.12 🟢 Volume tier → gpt-5.6-luna — shipped 2026-07-30.** Linked decision: [D-057](decisions.md#d-057--volume-tier-moves-to-gpt-56-luna-selection-is-now-measurable-2026-07-30). OpenAI's 80% Luna cut (verified: $0.20/$1.20) put it at input parity with `gpt-5.4-nano` and below it on output; a head-to-head on our own map batches showed 1.6-2x item yield, exact attribution (25 titles for 25 videos vs nano's 52 variants), and no JSON failures (nano failed 1 of 2). All 9 nano sites moved. Measured `MODEL_MAX_OUTPUT_TOKENS` for the 5.6 family (128K) — without it Luna silently fell back to the 4,096 default and clamped the map budget. Cost calculator corrected to the post-D-056 pipeline: benchmark $4.36 → $5.88.
- **S-1.14.13 🔵 Improve `search_rank_and_curate` — now measurable for the first time.** With S-1.14.6 live, a fresh search on the reference criteria captured **pool 651 → 200 selected / 451 rejected**. The rejects expose concrete ranking defects: only **12 of 30 preferred channels** are represented even though **14 of the 18 absent ones had videos sitting in the rejected pool** (@fireship 7, @matthew_berman 7, @theprimetimeagen 7, @openai 6, @aiexplained-official 6, @dwarkeshpatel 5, @google_deepmind 5, @twominutepapers 5, @lexfridman 3 …), while **IBM Technology — not a preferred channel — takes 10.5% of the corpus**. The ranker is not honouring the preferred-channel instruction.
  - [ ] T-1.14.13.1 Make preferred-channel coverage an explicit ranking objective (guarantee ≥1 video per resolved preferred channel before filling the remainder on merit)
  - [ ] T-1.14.13.2 Cap per-channel share of the final selection so no single non-preferred channel dominates
  - [ ] T-1.14.13.3 Re-rank the captured 651-candidate pool with the revised prompt and A/B against the shipped 200 (now possible — this is what S-1.14.6 bought)
  - [ ] T-1.14.13.4 Drop sub-5-minute shorts and non-English titles at the filter stage rather than relying on the ranker
