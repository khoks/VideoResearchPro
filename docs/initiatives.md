...
- **S-1.14.14 🟡 Close the residual report gap — all three defects are prompt/retry, not model tier.** Fixes shipped 2026-07-31 — PR [#209](https://github.com/khoks/VideoResearchPro/pull/209); re-measurement in flight.
  - [x] T-1.14.14.1 `COMPOSE_SECTION_PROMPT` requires the source in the VISIBLE citation text (`Video Title — Channel Name (<a>MM:SS</a>)`), link *in addition to* the label. 75% of citations rendered as a bare `( 12:34 )`; scored 6 vs 9 on attribution despite all 1,489 anchors carrying correct URLs
  - [x] T-1.14.14.2 `COMPOSE_SECTION_PROMPT` forbids sub-sections named after a channel/speaker; requires grouping by theme/debate/claim with sources synthesised into shared passages and disagreements made explicit — the largest score gap (insight 5 vs 9)
  - [x] T-1.14.14.3 `_invoke_with_retry` (5/20/45s backoff) on section + summary calls, with an error classifier — 429/5xx/overloaded/timeout retry; context-length, invalid-request and auth fail fast. A `529 Overloaded` had cost an entire report section
  - [x] T-1.14.14.4 Executive-summary prompt marks statistics computed-not-estimated and requires size/count/date figures quoted exactly — a judge caught production mis-stating corpus size and vintage
  - [ ] T-1.14.14.5 Re-judge after the above, and only then decide whether `report_compose` warrants a model-tier change. **Decision rule set in advance:** if attribution and insight move into the 8s the gap was prompt-shaped and no upgrade is justified; if they stay at 5-6 that is genuine model headroom. `report_compose` is the largest line item in the benchmark ($3.03 of $7.65), so this is the most expensive change available and must be evidence-led
