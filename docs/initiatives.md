...
- [ ] **C5. 🟡 substantively partial** S-1.5.4 polymorphic `<ApprovalCard>` renders Reddit + HN config entries with badges + filter chips — *T-1.5.4.1 (TS shape) + T-1.5.4.2 (`<ClassificationBadgeRow>`) shipped earlier (PRs [#99](https://github.com/khoks/VideoResearchPro/pull/99), [#108](https://github.com/khoks/VideoResearchPro/pull/108)) + **T-1.5.4.1 full primitive build + T-1.5.4.5 Reddit/HN config entries** ✅ shipped 2026-05-02 PR [#118](https://github.com/khoks/VideoResearchPro/pull/118). The polymorphic `<ApprovalCard>` component + `<CardHeader>`/`<CardActions>`/`<CardBody>`/`<CardMetaRow>` sub-components + FORMATTERS hybrid registry + SOURCE_CONFIGS mapped-type registry (video / reddit_post / hn_story) all in. **Remaining**: T-1.5.4.3 filter rail UI + T-1.5.4.4 YouTube approval-page migration to consume the polymorphic primitive*
...
- [ ] **C7.** End-to-end pipeline test passes for `["reddit_post"]`, `["hn_story"]`, and mixed `["video","reddit_post","hn_story"]` — *Reddit-only integration test ✅ via PR [#116](https://github.com/khoks/VideoResearchPro/pull/116) (`test_execute_topic_job_reddit_only_goes_through_dispatch_path`). HN-only + mixed e2e tests still pending; full closure also wants an HTTP-level integration through the routers*

**Progress as of 2026-05-02 (end-of-day):** **5 of 7 fully closed (C1, C2, C3, C4, C6); 1 of 7 substantively partial (C5: primitive built, page integration remains); 1 of 7 partial (C7: 1/3 e2e tests written)**. Was 0/7 at session start; **net +5 in this push**.

The remaining work to fully close M-1.5:

1. **C5 page integration**: T-1.5.4.3 filter rail (~"Show low-relevance" toggle + filter chips operating on `source_metadata.<field>`) + T-1.5.4.4 migrate ` JobDetailPage` approval surface to consume `<ApprovalCard>` + `SOURCE_CONFIGS`. Frontend-only.

2. **C7 remaining e2e tests**: HN-only + mixed `["video","reddit_post","hn_story"]` e2e tests. Same pattern as the existing Reddit-only test; ~30 min of work each.

3. **C4 polish (optional)**: T-1.5.3.5 classifier golden tests (sarcasm / sincere praise / in-favor / against fixtures) + T-1.5.3.6 framing exemplars baked literally into the LLM prompt (currently the exemplars live in source-types.md but the prompt itself is a stub). Closes the last threads on C4.

4. **Backend reference enrichment (post-M-1.5)**: Teach `extract_references` LLM use case to emit `source_type` + `permalink` + `author` per source so Reddit / HN citations actually render with their proper labels in production (the frontend rendering contract is in place; the backend producer is the missing half).
